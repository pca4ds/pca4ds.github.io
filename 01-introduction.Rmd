...
| 29  | `teacher`               | School teacher                       |
| 30  | `bus_driver`            | Bus driver                           |
| 31  | `mechanic`              | Car mechanic                         |
| 32  | `construction_worker`   | Construction worker                  |
| 33  | `metalworker`           | Metalworker                          |
| 34  | `cook_chef`             | Cook chef                            |
| 35  | `departmental_head`     | Departmental head                    |
| 36  | `engineer`              | Engineer                             |
| 37  | `bank_clerk`            | Bank clerk (cashier)                 |
| 38  | `executive_secretary`   | Executive secretary                  |
| 39  | `salesperson`           | Salesperson (sales associate)        |
| 40  | `textile_worker`        | Textile worker                       |
...
regarding a data matrix from the dual point of view of rows oand columns. 
...
knitr::include_graphics("images/cloud-rows-in-factorial-plane.png")
```
...
We can consider of projecting this pencil cloud over different planes. 
...
\begin{equation}
\sum_{i} d^{2} (i, G) = Max_H d^{2}_{H} (i, G) + \sum_i d^{2}_{\bar{H}} (i, G)
(\#eq:4)
\end{equation}
...
Let's discuss what the analysis involves regarding the cloud of variable-points. Recall that the cloud of variable-points is defined by the columns of the data matrix $\mathbf{X}$. Without loss of generality, we will assume that the variables are mean-centered and standardized.
...
Interestingly, both cloud of points---invidiauals and variables---have the same inertia. On one hand, the inertia of the cloud of row-points is the sum of the squared distances between each point and the center of gravity, weighed by the importance of each individual. This inertia can be expressed with respect to each axis in the original space (in which each axis corresponds to a variable):
...
The natural metrix matrix to be used in this case is the diagonal matrix $\mathbf{N}$ of weights for individuals ($1/n$ or $p_i$). Thus, the distance between variables is defined as:
...
which coincides with \@ref(eq:17) when using mena-centered data and standardized (normalized PCA), or with \@ref(eq:16) when using non-centenred data (non-normalized PCA).
