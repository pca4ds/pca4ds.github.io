...
__INSERT TABLE 3.3 (see page 88)__
...
where the quantities $x_{ij}$ express the rank of the individual $i$ of teh $j$ variable. The advantage of the Spearman's correlation coefficient is that coincides with the Pearson coeffcient (usual correlation) when applied to a matrix of ranks.

In the case of the international cities, the Spearman's correlations are given in table \@ref(tab:table-3-6).
...
knitr::kable(
  ...
  booktabs = TRUE,
  caption = "Spearman's Rank Correlation matrix of variables (2nd analysis)"
)
```
...
knitr::kable(
  table_3_8,
  booktabs = TRUE,
  caption = 'PCA results on table of ranks (second analysis)'
)
```
