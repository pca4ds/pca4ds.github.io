...
The idea behind the so-called V-test involves performing a hypothesis test. The null hypothesis $H_0$ consists in the assumption that a set of $_k$ individuals are randomly selected, without replacement, from the total of $n$ individuals.
...
If $n$ and $n_k$ are not very small, the central limit theorem si applicable (even though the extractions are not indipendent) and in this case the variable:
...
In our working examples of the cities, we have a nominal categorical variable: the region of the world in which a city is located. This variable allows us to obtain a simplified representation of the cloud of cities. The results obtained with the first three axes are displayed in __TABLE 2.11__.

__INSERT TABLE 2.1__

_Caption: V-test of the variable `world region`, used in the second PCA._

The first column, names `EFF`, provides the effective of each category (total number of cities of each category). The second column, named `PABS` provides the weight (sum of the weights of all the cities in a given category). When we have uniform weights, the weight and the effective are the same. The first category is formed by six cities of Northern Europe.
